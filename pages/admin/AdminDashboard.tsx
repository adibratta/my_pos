...
const AdminDashboard: React.FC = () => {
    ...
    return (
      <div>
        <div className="flex justify-between items-center mb-6">
          <h2 className="text-2xl font-bold text-gray-800">Manajemen Produk & Stok</h2>
          <button onClick={() => { setFormData({ type: 'READY', stock: 0, price: 0, cost: 0 }); setIsEditing(true); }} className="bg-blue-600 text-white px-4 py-2 rounded flex items-center gap-2 hover:bg-blue-700">
                ...
                <div className="flex justify-between items-center mb-4">
                     ...
                     <div className="flex gap-2 no-print">
                        <input 
                            ...
                            className="pl-3 pr-3 py-1.5 border rounded text-sm w-48"
                            value={searchTerm}
                            onChange={(e) => setSearchTerm(e.target.value)}
                        />
                        <button onClick={downloadPdf} className="text-sm bg-gray-800 text-white px-3 py-1 rounded hover:bg-gray-700">Export PDF</button>
                     </div>
                </div>
                <div className="bg-white rounded-lg shadow overflow-hidden">
        ...
        <div className="space-y-8">
            <div className="flex justify-between items-center no-print">
                ...
                <div className="flex gap-4">
                     <input type="month" value={month} onChange={e => setMonth(e.target.value)} className="border p-2 rounded bg-white" />
                     <button onClick={() => window.print()} className="bg-gray-800 text-white px-4 py-2 rounded flex items-center gap-2"><Printer size={16}/> Print / PDF</button>
                </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                <div className="bg-white p-4 rounded shadow border-l-4 border-blue-500">
                    ...
                    <p className="text-2xl font-bold text-blue-600">{settings.currencySymbol} {totalRevenue.toLocaleString()}</p>
                </div>
                <div className="bg-white p-4 rounded shadow border-l-4 border-green-500">
                    <p className="text-gray-500 text-sm">Laba Kotor</p>
                    <p className="text-2xl font-bold text-green-600">{settings.currencySymbol} {totalProfit.toLocaleString()}</p>
                </div>
                <div className="bg-white p-4 rounded shadow border-l-4 border-red-500">
                    <p className="text-gray-500 text-sm">Pengeluaran</p>
                    <p className="text-2xl font-bold text-red-600">{settings.currencySymbol} {totalExpense.toLocaleString()}</p>
                </div>
                <div className="bg-white p-4 rounded shadow border-l-4 border-purple-500">
                    <p className="text-gray-500 text-sm">Laba Bersih</p>
                    <p className="text-2xl font-bold text-purple-600">{settings.currencySymbol} {netProfit.toLocaleString()}</p>
                </div>
            </div>

            {/* Chart */}
            <div className="bg-white p-4 rounded shadow h-80 no-print">
            ...
            <div className="bg-gradient-to-r from-indigo-50 to-purple-50 p-6 rounded-lg border border-indigo-100 no-print">
                <div className="flex justify-between items-start mb-4">
                    ...
                    <button onClick={runAnalysis} disabled={loadingAi} className="bg-indigo-600 text-white px-4 py-2 rounded shadow hover:bg-indigo-700 disabled:opacity-50">
                        {loadingAi ? 'Menganalisis...' : 'Analisa Data Penjualan'}
                    </button>
                </div>
                {aiAnalysis ? (
                    <div className="prose text-sm text-indigo-800 whitespace-pre-line bg-white p-4 rounded border border-indigo-100">
                        {aiAnalysis}
                    </div>
                ) : (
            ...
            <div className="bg-white rounded shadow p-6">
                <div className="flex justify-between items-center mb-4">
                    <div className="flex gap-2 items-center">
                        ...
                        <div className="relative no-print">
                            ...
                            <input 
                                type="text" 
                                placeholder="Cari..." 
                                className="pl-7 pr-2 py-1 border rounded text-xs w-32"
                                value={searchTerm}
                                onChange={e => setSearchTerm(e.target.value)}
                            />
                        </div>
                    ...
                    <button 
                        ...
                        className="text-xs bg-red-100 text-red-600 px-2 py-1 rounded hover:bg-red-200 no-print"
                    >
                        + Tambah
                    </button>
                </div>
  ...
  return (
    <div className="flex min-h-screen bg-gray-100">
      {/* Sidebar */}
      <aside className="w-64 bg-gray-900 text-white flex flex-col no-print fixed h-full z-10">
        <div className="p-6 flex items-center space-x-3 border-b border-gray-800">
          <img src={settings.logo} className="w-10 h-10 rounded-full bg-white object-cover" alt="Logo" />
          <h1 className="font-bold text-lg leading-tight">{settings.name} <br/><span className="text-xs text-gray-400 font-normal">Admin Panel</span></h1>
        </div>
        
        <nav className="flex-1 p-4 space-y-2 overflow-y-auto">
          <TabButton active={activeTab === 'overview'} onClick={() => setActiveTab('overview')} icon={LayoutDashboard} label="Produk & Stok" />
          <TabButton active={activeTab === 'transactions'} onClick={() => setActiveTab('transactions')} icon={History} label="PO & Transaksi" />
          <TabButton active={activeTab === 'reports'} onClick={() => setActiveTab('reports')} icon={FileText} label="Laporan & AI" />
          <TabButton active={activeTab === 'settings'} onClick={() => setActiveTab('settings')} icon={Settings} label="Pengaturan" />
        </nav>
      ...
      {/* Main Content */}
      <main className="flex-1 ml-64 p-8 overflow-y-auto h-screen print-ml-0">
